...
const unsigned long SectSWDelayTime = 700; //time the arduino waits after manual Switch is used before acception command from AOG in msec
...
byte SectMainToAOG = 0, SectMainToAOGOld = 0;
...
void loop()
{
	...
	//timed loop
	if ((currentTime - lastTime >= LOOP_TIME) || NewDataToAOG)
	{
		NewDataToAOG = false;
  ...
  //****************************************************************************************
	...
	if ((RelayToAOG[0] != RelayToAOGOld[0]) || (RelayToAOG[1] != RelayToAOGOld[1])
		...
		|| (SectMainToAOG != SectMainToAOGOld))
	{
		NewDataToAOG = true;
	}
